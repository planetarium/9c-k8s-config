...
def update_internal_manifests(
    repo_infos: List[Tuple[str, str, str]], apv: Apv
):
    if config.env == "test":
        branch = f"test-rc-v{apv.version}"
    else:
        branch = f"rc-v{apv.version}"

    update_headless(repo_infos, branch)
    update_apv(apv.raw, branch)


def update_main_manifests(repo_infos: List[Tuple[str, str, str]], apv: Apv):
    pass
    ...
    with open(os.path.join(INTERNAL_DIR, "kustomization.yaml")) as f:
        ...
        for image in doc["images"]:
            if image["name"] == "kustomization-ninechronicles-headless":
                ...
                image[
                    "newTag"
                ] = f"git-{repo_map['NineChronicles.DataProvider']}"
        new_doc = yaml.safe_dump(doc)
        commit_manifests(branch, IMAGE_PATH, new_doc)


def update_apv(apv: str, branch: str):
    with open(os.path.join(INTERNAL_DIR, "configmap-versions")) as f:
        doc = yaml.safe_load(f)
