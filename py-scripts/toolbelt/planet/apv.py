...
from toolbelt.config import MAINNET_ADDRESS, INTERNAL_ADDRESS
...
class Planet(ApvParser):
    ...
    def _get_key_id(self, nc_network: str) -> str:
        assert (
            nc_network == "main" or nc_network == "internal"
        ), f"{nc_network}"
        ...
        for i in range(0, len(key_address), 2):
            ...
            if (
                nc_network == "main"
                and address == MAINNET_ADDRESS
            ) or (
                nc_network == "internal"
                and address == INTERNAL_ADDRESS
            ):
                return key_id
    ...
    def sign_apv(self, nc_network: str, version: int, timestamp: str) -> Apv:
        ...
        out = subprocess.run(
            command_str, capture_output=True, text=True, shell=True
        )
        if not out.stdout:
